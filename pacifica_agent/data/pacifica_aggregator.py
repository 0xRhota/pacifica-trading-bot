"""
Pacifica Market Data Aggregator
Uses Pacifica DEX data instead of Pacifica

Symbols fetched dynamically from Pacifica API via SDK
"""
...
from pacifica_agent.data.pacifica_fetcher import PacificaDataFetcher

logger = logging.getLogger(__name__)


class PacificaMarketDataAggregator:
    """
    Market data aggregator for Pacifica DEX
    Uses Pacifica API for OHLCV data, not Pacifica
    ...
    def __init__(
        ...
        """
        Initialize Pacifica market data aggregator

        Args:
            cambrian_api_key: Cambrian API key for Deep42 (macro context only)
            sdk: PacificaSDK instance for fetching market metadata
        ...
        # Pacifica data fetcher with SDK for dynamic symbol loading
        self.pacifica = PacificaDataFetcher(sdk=sdk)
        ...
        logger.info(f"✅ PacificaMarketDataAggregator initialized (interval={interval}, candles={candle_limit})")

    @property
    def pacifica_markets(self) -> List[str]:
        """
        Get list of available Pacifica markets from fetcher (loaded dynamically from API)

        Returns:
            List of symbol strings
        """
        return self.pacifica.available_symbols

    async def fetch_market_data(self, symbol: str) -> Optional[Dict]:
        """
        Fetch comprehensive market data for a single Pacifica symbol (async)

        Args:
            symbol: Pacifica symbol (dynamically loaded from API)

        Returns:
            Dict with all market data, or None if symbol not available on Pacifica
        """
        # Check if symbol is available on Pacifica (from API)
        if symbol not in self.pacifica_markets:
            logger.warning(f"Symbol {symbol} not available on Pacifica (available: {', '.join(self.pacifica_markets)})")
            return None

        # Fetch Pacifica data using HTTP API (not SDK)
        # Note: PacificaDataFetcher uses HTTP API directly, doesn't need candlestick_api

        pacifica_data = await self.pacifica.fetch_market_data(
            symbol=symbol,
        ...
        if not pacifica_data:
            logger.warning(f"No data returned from Pacifica for {symbol}")
            return None
        ...
        # Fetch OI (may not be available for all Pacifica markets)
    ...
    async def fetch_all_markets(self, symbols: Optional[List[str]] = None) -> Dict[str, Dict]:
        """
        Fetch market data for all Pacifica markets (async)

        Args:
            symbols: List of symbols (default: all Pacifica markets from API)

        Returns:
            Dict mapping symbol -> market data dict
        """
        if symbols is None:
            symbols = self.pacifica_markets

            # LIQUIDITY FILTER: Only trade high-volume crypto, exclude illiquid forex/commodities
            # Forex pairs (low liquidity on Pacifica)
            ...
            logger.info(f"🔍 Liquidity filter: {len(self.pacifica_markets)} total markets → {len(symbols)} liquid crypto markets (excluded {len(excluded)} illiquid)")
        else:
            # Filter to only Pacifica markets (from API)
            symbols = [s for s in symbols if s in self.pacifica_markets]
            if not symbols:
                logger.warning(f"No valid Pacifica symbols in provided list")
                return {}

        # Use fetcher's async method (uses HTTP API directly)
        # Note: PacificaDataFetcher doesn't need candlestick_api
        results = await self.pacifica.fetch_all_markets(
            symbols=symbols,
            interval=self.interval,
            limit=self.candle_limit,
            candlestick_api=self.candlestick_api,
