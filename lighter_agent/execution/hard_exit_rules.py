...
class HardExitRules:
    ...
    def should_prevent_close(
        ...
        # If under min hold AND not at target/stop, prevent close
        if hold_hours < self.min_hold_hours:
            if abs(pnl_pct) < self.profit_target_pct and abs(pnl_pct) < self.stop_loss_pct:
                reason = f"Min hold {self.min_hold_hours}h not met (held {hold_hours:.1f}h, P&L {pnl_pct:+.2f}%)"
                logger.info(f"🔒 PREVENTING CLOSE: {reason}")
                return True, reason
